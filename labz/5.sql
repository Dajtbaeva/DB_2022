...
select warehouse, count(value) from packs group by warehouse;

-- 8. Same as previous exercise, but select only those warehouses where the number of the packs is greater than 2.
select warehouse, count(value) from packs group by warehouse having count(value) > 2;
...
insert into packs (code, contents, value, warehouse) values ('H5RT', 'Papers', 150, 2);

-- 11. Reduce the value of the third largest pack by 18%.
update packs set value = value * 0.82 where value in (select distinct value from packs order by value desc limit 1 offset 2);
...
delete from packs where warehouse in (select code from warehouses where location = 'Chicago') returning *;
